...
def locsim(l1,l2):
    w1 = df.loc[l1, WEIGHT_COL].sum()
    w2 = df.loc[l2, WEIGHT_COL].sum()
    kpicorr = dfmult(matnorm(df.loc[l1, kpis]).T, matnorm(df.loc[l2, kpis]))
    return w1 * w2 * np.diag(kpicorr).sum()    
def wlocsim(l1, l2):
    kpicorr = dfmult(matnorm(df.loc[l1, wkpis]).T, matnorm(df.loc[l2, wkpis]))
    return np.diag(kpicorr).sum()
#%%
lh,*lT = locs
K = 4
gps = [[lh],*[[] for _ in range(1,K)]]
locsims = [wlocsim(lh,lt) for lt in tqdm.tqdm(lT)]
for i,(_,l) in enumerate(sorted(zip(locsims,lT),reverse=True)):
    gps[(i+1)%K].append(l)
#%%
from matplotlib import pyplot as plt
def gpagg(df, gp,agg_args=["sum"],agg_kwargs={}):
    return df \
        .reset_index(level=2) \
        .loc[gp] \
        .groupby("date") .agg(*agg_args,**agg_kwargs)

def gpcorr(df,gpi,gpj):
    X = gpagg(df,gpi)[wkpis]
    Y = gpagg(df,gpj)[wkpis]
...
def score_split(df,gps):
    ...
    for i in range(len(gps)):
        for j in range(i+1,len(gps)):
            score += gpcorr(df,gps[i],gps[j]).sum()
    return score
...
for i in range(len(gps)):
      for j in range(i+1,len(gps)):
            print(i,j,gpcorr(df, gps[i],gps[j]))
#%%
for c in wkpis:
    ax = plt.gca()
    for i,gp in enumerate(gps):
        matnorm(gpagg(df,gp))[c].plot(figsize=(20,10),label=f"gp {i}")
